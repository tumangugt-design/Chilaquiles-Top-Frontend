...
const InternalOrder = ({ onSuccess }) => {
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 'BASE_RECIPE':
        return <BaseRecipePage plate={order.currentPlate} plateNumber={order.cart.length + 1} updatePlate={updateCurrentPlate} onNext={nextStep} onBack={prevStep} showUnavailable order={order} updateOrder={updateOrder} />
      case 'SUMMARY':
