...
export const deleteInventoryItem = (name, reason) => api.delete(`/inventory/${encodeURIComponent(name)}`, { data: { reason } })
export const renameInventoryItem = (name, newName, reason) => api.patch(`/inventory/${encodeURIComponent(name)}/rename`, { newName, reason })
...
export const getSuppliers = () => api.get(`/suppliers?t=${Date.now()}`)
...
// Fase 2 (Compras/Lotes)
export const getPurchases = (ingredientName) => api.get(`/purchases?t=${Date.now()}${ingredientName ? `&ingredientName=${encodeURIComponent(ingredientName)}` : ''}`)
...
export const createPurchaseAllocation = (purchaseId, payload) => api.post(`/purchases/${purchaseId}/allocations`, payload)
